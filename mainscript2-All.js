...
var stateOne = {x: 0, prevX: 0}			//текущее состояние
maxDelay = {all: 0, cumulative: 0};							//максимальная задержка на остановке
busCount = 0;

var starttimes;
var starttimestext;

drawMode = ["all","cumulative"];								//задаем вид графика
...
function updateStopRadiusOne(mode){

	console.log("stop states one while updating", delaysStateOne, mode)
	...
	console.log("delays state one", delaysStateOne)
	for(stop=0; stop<delaysStateOne.length; stop++){


		deltaR = 0
		for(driver=0; driver<delaysStateOne[stop].length; driver++){
			//console.log("calculating radius", stop, driver, deltaR)
			if(delaysStateOne[stop][driver]==1){							//увеличиваем радиус на
				deltaR += busData[driver].values[1][stop].late;
				//console.log("updating radius of stop "+stop+" on "+deltaR);
				delaysStateOne[stop][driver] = 2;
			} else if (delaysStateOne[stop][driver]==-1) {
				deltaR -= busData[driver].values[1][stop].late;
				delaysStateOne[stop][driver] = -2;
			}
		...
		if(deltaR){
			...
			oldR = d3.select(rootObject+" #delay"+stop).attr("r");

			oldR = (oldR/maxRadius)*maxDelay[mode];
			//console.log("oldR", oldR, maxRadius, maxDelay[mode], stop);
			newR = oldR + deltaR;
			newR = (newR/maxDelay[mode])*maxRadius; if(newR<0){ newR = 0;}

			d3.select(rootObject+" #delay"+stop)
			  //.transition()
			  //.duration(100)
			  .attr("r",newR);

			//console.log("updating", d3.select(rootObject+" #delay"+stop)[0], oldR, newR);
...
function updateStopRadius(mode){
	...
	console.log("stop states while updating", delaysState)

	//console.log("["+mode+" mode] updating stops");
	...
	rootObject = roots[mode];

	console.log("delays state", delaysState);
	...
	for(stop=0; stop<delaysState.length; stop++){
		...
		for(driver=0; driver<delaysState[stop].length; driver++){
			...
			if(delaysState[stop][driver]==1){							//увеличиваем радиус на
				...
				if(mode!="cumulative") {
					...
					//console.log(maxDelay, mode)
					d3.select(rootObject+" .delay"+driver+" #delay"+stop)
					  .transition()
					  .duration(500)
					  .attr("r",(busData[driver].values[1][stop].late/maxDelay[mode])*maxRadius);
				}
		...
		if(deltaR && mode=="cumulative"){
			...
			oldR = d3.select(rootObject+" #delay"+stop).attr("r");
			console.log("checking oldR", oldR);
			oldR = (oldR/maxRadius)*maxDelay[mode];
			newR = oldR + deltaR;
			newR = (newR/maxDelay[mode])*maxRadius; if(newR<0){ newR = 0;}
...
function updateStopStatesOne(x){

	console.log("FUNC: updateStopStatesOne, stop states", delaysState, delaysStateOne)
	...
	for(stop=0; stop<delaysStateOne.length; stop++){
		...
		for(driver=0; driver<delaysStateOne[stop].length; driver++){
			...
			if((curstopX <= x[driver][0] && delaysStateOne[stop][driver]==0) || (curstopX<=x[driver][0] && delaysStateOne[stop][driver]==-2)){
				delaysStateOne[stop][driver] = 1;
			} else if (curstopX>x[driver][0] && delaysStateOne[stop][driver]==2){
				delaysStateOne[stop][driver] = -1;
			}
		}

	}
	console.log("new delays states",delaysStateOne);
}
...
function updateDrivers(xData){
	console.log("["+curmode+" mode] updating drivers")
	roots = {
		all: ".svgAll",
		cumulative: ".svgOne"
	}

	rootObject = roots[curmode];
	//coords = getX(time);

	b = d3.selectAll(rootObject+" .drivers rect")
	  .data(xData)
	  .attr("x",function(d){
	 	return d[0]+xOffset;
	  })
...
function prepareGraph(mode){
	...
	basicOffset = 20; if(mode=="cumulative") {basicOffset = 0;}
	stopOffset = 28; if(mode=="cumulative") {stopOffset = 0;}
	speedOffset =25; if(mode=="cumulative") {speedOffset = 25;}

	//var myGlow = glow("myGlow").rgb("#f00").stdDeviation(3);

	console.log("["+mode+" mode] setting svg");

	svg = d3.select(rootObject)
			.attr("width", 900)
			.attr("height", function(){
				if(mode=="cumulative"){ return 100; }
				else {return basicOffset+speedOffset*18+stopOffset+50;}
			})
			//.call(myGlow);

	if(mode!="cumulative"){
		var stopTicks =    	  d3.select(rootObject)										//рисуем остановки
								.append("g")
								.attr("class","stop")
								.selectAll("rect")
		...
		i=0;
		var startTimesList = d3.select(rootObject)
								.append("g")
		...
		var startTimesList = d3.select(rootObject)
								...
								.append("text")
								.attr("x",0)
								.attr("y",basicOffset+speedOffset+stopOffset-12)
								.attr("dy", ".35em")
								.text("Все")

	}

	var gradientlist = d3.select(rootObject)
							.append("g")
							.attr("class","gradientlist")

	i=0;
	ostanovka = false;
	var text = 	 d3.select(rootObject)
							.append("g")
							.attr("class","stoptext")
		 					.selectAll("text")
		 					.data(busData[0].values[1])
...
function emptyData(){
	...
	delaysStateOne = new Array(busData[0].values[1].length);

	for(i=0; i<busData[0].values[1].length; i++){
		delaysStateOne[i] = new Array(busData.length);
	}

	for(i=0; i<delaysStateOne.length; i++){
		for(j=0; j<delaysStateOne[i].length; j++){
			delaysStateOne[i][j] = 0;
		}
	}
...
function drawGraph(mode){
	...
	console.log("["+mode+" mode] drawing graph");
	...
	rootObject = roots[mode];
	rootObject2 = roots["cumulative"];


	basicOffset = 20; if(mode=="cumulative") {basicOffset = 0;}
	stopOffset = 28; if(mode=="cumulative") {stopOffset = 10;}
	speedOffset =25; if(mode=="cumulative") {speedOffset = 0;}
	modeexcluder = 25;  if(mode=="cumulative") {modeexcluder = 0;}
	...
	busData.forEach(function(d){
		...
		var speedRects = 	  d3.select(rootObject+" .routes")											//рисуем интервалы скорости
								.append("g")
								.attr("class",curdate)
								.selectAll(".speed"+i)
								.data(intervals)
								...
								.call(function(d,i){									//создаем градиенты для раскраски
									...
									intervals.forEach(function(p){
										...
										var gradient = d3.select(rootObject+" .gradientlist")
												  .append("svg:defs")
								...
								.attr("class","speed"+i)
								.attr("x",function(d){return d.x1+xOffset})
								.attr("y",basicOffset+speedOffset*(i+1)+stopOffset-modeexcluder)
		...
		if(mode!="cumulative"){
			...
			var timeoverlay = 	   d3.select(rootObject+" ."+curdate)
						.append("rect")
			...
			var whiteCircles =	  d3.select(rootObject+" .whitespots")
									.selectAll(".whitestop")
									.data(stops)
									.enter()
									.append("circle")
			...
			var ground = 	  d3.select(rootObject+" ."+curdate)
			...
			var delayCircles =	  d3.select(rootObject+" .delaylist")											//Если режим наложения, нарисуем кружки после цикла
									.append("g")
									.attr("class","delay"+i)
									.selectAll(".delay")
									.data(stops)
									...
									.attr("fill","#f00")
						  			...
						  			.attr("maxR", function(d){
						  				return d.late/maxDelay[mode]*maxRadius;
						  			})
		}

		var driverList = 	  d3.select(rootObject+" .drivers")										//добавляем отметки водителей
								.append("rect")
								.attr("id","driver"+i)
								.attr("x",xOffset)
								.attr("y",basicOffset+speedOffset*(i+1)+stopOffset-modeexcluder)
	...
	if(mode=="cumulative"){
		...
		var ground = 	   	 d3.select(rootObject2)
								.append("rect")											//добавляем горизонтальную черту под графиком
	      			  			.attr("x",xOffset)
					  			.attr("y",basicOffset+stopOffset+2)
					  			.attr("width", width)
					  			.attr("height",1)
					  			.attr("fill","#000")
					  			.attr("opacity", 0.15)
		...
		delayNum = 0
		var delayCircles =	  d3.select(rootObject2)
										.append("g")
										.attr("class","delay")
										.selectAll(".delay")
										.data(stops)
										...
										//.attr("time", function(d){return d.t1})
										.attr("cx", function(d){return d.x1+xOffset})
										.attr("cy", basicOffset+speedOffset*(i+1)+stopOffset+3)
										.attr("fill","#f00")
							  			.attr("stroke","rgba(255, 255, 255, 0.5)")
							  			.attr("r",0)
							  			.attr("maxR", function(d){
...
function updateOverlaysAndStopName(mode,xData){
	...
	console.log("["+mode+" mode] updating overlays");
	roots = {
		all: ".svgAll",
		cumulative: ".svgOne"
	}
	...
	if(mode=="cumulative"){
		...
		d3.select(rootObject+" #timeoverlay")
		...
		xData.forEach(function(d){
			...
			d3.select(rootObject+" #timeoverlay"+i)
...
function slider(){
	...
	function mousemove(){
		...
		curmode = 'all';
		updateOverlaysAndStopName(curmode,xData);
		updateDrivers(xData);
		updateStopStates(xData);
		updateStopRadius(curmode);
		curmode = 'cumulative';
		updateOverlaysAndStopName(curmode,xData);
		updateDrivers(xData);
		updateStopStatesOne(xData);
		updateStopRadiusOne(curmode);
		//console.log(xData);
	}
...
function setDefault(){
	...
	if(currentX<0){ currentX=0} else if(currentX>720) {currentX=720}
		...
		/*
		updateOverlaysAndStopName(curmode,xData);
		updateOverlaysAndStopName("cumulative",xData);
		updateDrivers(xData);
		console.log("what is xData", xData);
		updateStopStates(xData);
		updateStopStatesOne(xData);
		console.log("stop states after update", delaysState, delaysStateOne)
		//console.log("["+curmode+" mode] preparing to update radius")
		updateStopRadius(curmode);
		updateStopRadiusOne("cumulative");
		//console.log("stop states one before update", delaysStateOne)
		//updateStopStatesOne(xData);
		//updateStopRadiusOne("cumulative");
		*/
...
d3.csv('busdata-temp.csv', function (data) {
	...
	console.log("prepared busdata", busData);
	...
	maxDelay['all'] = findDelay(curmode);
	...
	prepareGraph(curmode);									//делаем SVG нужного размера
	drawGraph(curmode);
	//setDefault();
	//setDefault();

	curmode = "cumulative";
	maxDelay['cumulative'] = findDelay(drawMode[1]);
	prepareGraph(curmode);									//делаем SVG нужного размера
	drawGraph(curmode);
	setDefault();
