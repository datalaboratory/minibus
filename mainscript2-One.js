...
function getStopsTime(){
	for(stopI=0; stopI<stoptimes.length; stopI++){
		for(drivI=0; drivI<stoptimes[stopI].length; drivI++){
			...
			//console.log("check stoptimes", stoptimes[stopI][drivI], Number(busData[drivI].values[1][stopI].t1) + starttimes[drivI])
		}
	}
...
function getStartText(){
	...
	starttimes.forEach(function(d){
		...
		starttimestext[i] = newH+" м "+newM+" с";
		i++;
	})
...
function getFacts(mode){
	...
	averageDelay = Math.round(delaySum/count);
...
function initDefault(){
	...
	d3.select("#timeoverlay")
		 	  .attr("x",getX(currentTime,daydata)[0]);
...
function prepareGraph(){
	...
	console.log("avgStopIntervals", avgStopIntervals);
	...
	var text = 	 d3.select("svg")
							...
							.text(function(d){
								...
								return d.stopname.substr(d.stopname.indexOf(']')+2,1000)
							})
							.attr("dy", ".35em")
							.attr("opacity",0)
...
function drawGraph(mode){
	...
	busData.forEach(function(d){
		...
		var speedRects = 	  d3.select(".routes")											//рисуем интервалы скорости
								...
								.attr("class","speed"+i)
								.attr("x",function(d){return d.x1;})
								.attr("y",basicOffset+speedOffset*(i+1)+stopOffset)
		...
		if(mode!="cumulative"){
			...
			var ground = 	   	 d3.select("."+curdate)
								.append("rect")											//добавляем горизонтальную черту под графиком
	      			  			.attr("x",0)
					  			.attr("y",basicOffset+speedOffset*(i+1)+stopOffset+4)
					  			.attr("width", width)
					  			.attr("height",1)
					  			.attr("fill","#000")
					  			.attr("opacity", 0.01)
			...
			var delayCircles =	  d3.select("svg")											//Если режим наложения, нарисуем кружки после цикла
									.append("g")
									.attr("class","delay"+i)
									.selectAll(".delay")
									.data(stops)
	...
	if(mode=="cumulative"){
		...
		var timeoverlay = 	   d3.select(".whitespots").append("rect")
					    .attr("id","timeoverlay")
	      			    .attr("x",0)
		...
		var ground = 	   	 d3.select(".whitespots")
								.append("rect")											//добавляем горизонтальную черту под графиком
	      			  			.attr("x",0)
		...
		delayNum = 0
		var delayCircles =	  d3.select(".delaylist")
										.append("g")
										.attr("class","delay")
										.selectAll(".delay")
										.data(stops)
...
function slider(){
	...
	function mousemove(){
		currentX = d3.mouse(svg.node())[0];
		if(currentX<0){ currentX=0} else if(currentX>720) {currentX=720}
		time = (currentX/720)*maxTime;
		xData = getX(time);

		d3.select(".slider").style("left",currentX-31)
		d3.select(".text").style("left",currentX-13)
...
d3.csv('busdata-temp.csv', function (data) {
	...
	getFacts();
	prepareGraph();											//делаем SVG нужного размера
